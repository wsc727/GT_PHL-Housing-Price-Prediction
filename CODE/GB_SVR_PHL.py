...
#Read in data
assessment = fp.ParquetFile('PHL_assessment_clean.parq').to_pandas()
...
#additional cleaning to remove columns with a lot of nulls and other unnecessary data. Also reduce sales price range to exclude outliers
assessment.drop(columns=['Date_BLS','Date_prime','Date_Stock'],inplace=True)
assessment_clean = assessment[assessment['Building_Category']!='Multi Family']
assessment_clean = assessment_clean[(assessment_clean['Sale_Price']<=1500000) & (assessment_clean['Sale_Price']>=10000)]
assessment_clean = assessment_clean[assessment_clean['Non_Basement_Area']<15000]
assessment_clean.drop(columns=['Construction_Quality','Unit'],inplace=True)

#create final dataset that will be split into train and test; additional cleaning to convert categories to numbers
assessment_train = assessment_clean.copy()
assessment_train['Zip_Code_new'] = assessment_train['Zip_Code'].str[0:5]
assessment_train['Air_Conditioning_n'] = np.where(assessment_train['Air_Conditioning']=='None',0,1)
assessment_train['Physical_Condition'] = assessment_train['Physical_Condition'].astype(int)
assessment_train['Year_Built'] = assessment_train['Year_Built'].astype(int)
assessment_train['Aux_Unit_n'] = np.where(assessment_train['Aux_Unit']==False,0,1)
assessment_train['year'] = assessment_train['Sale_Date'].dt.year
assessment_train['month'] = assessment_train['Sale_Date'].dt.month
assessment_train.drop(columns=['Air_Conditioning','Building_Category','Property_Address','Aux_Unit','Parcel_ID','Sale_month','lat','lng','the_geom_text','Elevation_Type','Zip_Code','Sale_Date'],inplace=True)
